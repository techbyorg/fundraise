...
export default $org = function({irsOrgStream}) {
  const {model, lang} = useContext(context);

  var {irsOrg990StatsStream, metricValueStreams, contributionsStream,
    irsOrg990StatsAndMetricStream} = useMemo(function() {
  ...
  const {me, contributions, metric, irsOrg, irsPersons, graphData, irsOrg990Stats,
    org} = useStream(() => ({
    me: model.user.getMe(),
